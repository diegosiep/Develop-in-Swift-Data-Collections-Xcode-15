...
class HomeCollectionViewController: UICollectionViewController {
    ...
    func createDataSource() -> DataSourceType {
        ...
        let habitRegistration = UICollectionView.CellRegistration<LeaderboardHabitCollectionViewCell, ViewModel.Item.ID>(cellNib: habitNib) { [weak self] cell, indexPath, itemIdentifier in
            guard let self,
                  ...
                  case let .leaderboardHabit(name, leadingUserRanking, secondaryUserRanking) = item else {
                return
            }
        ...
        let userNib = UINib(nibName: "FollowedUserCollectionViewCell", bundle: Bundle(for: FollowedUserCollectionViewCell.self))
        let userRegistration = UICollectionView.CellRegistration<FollowedUserCollectionViewCell, ViewModel.Item.ID>(cellNib: userNib) { [weak self] cell, indexPath, itemIdentifier in
            guard let self,
                  let item = items.first(where: { $0.id == itemIdentifier}),
                  case let .followedUser(user, message) = item else {
                return
            }
        ...
        let dataSource = DataSourceType(collectionView: collectionView) { [weak self] collectionView, indexPath, itemIdentifier in
            ...
            case .followedUser:
                return collectionView.dequeueConfiguredReusableCell(using: userRegistration, for: indexPath, item: itemIdentifier)
            }
        }
        
        let leaderboardHeaderRegistration = UICollectionView.SupplementaryRegistration<NamedSectionHeaderView>(elementKind: SupplementaryView.leaderboardSectionHeader.viewKind) { header, elementKind, indexPath in
        ...
        let followedUsersHeaderRegistration = UICollectionView.SupplementaryRegistration<NamedSectionHeaderView>(elementKind: SupplementaryView.followedUsersSectionHeader.viewKind) { header, elementKind, indexPath in
        ...
        dataSource.supplementaryViewProvider = { collectionView, kind, indexPath in
            ...
            case .followedUsersSectionHeader:
                return collectionView.dequeueConfiguredReusableSupplementary(using: followedUsersHeaderRegistration, for: indexPath)
            default:
                return nil
            }
        }
    ...
    func updateCollectionView() {
        ...
        itemsBySection[.followedUsers] = followedUserItems.map(\.id)
